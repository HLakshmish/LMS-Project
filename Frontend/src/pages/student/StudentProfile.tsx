...
const StudentProfile: React.FC<StudentProfileProps> = ({ user: propUser, onLogout }) => {
  const { user, isLoading: authLoading } = useAuth();
  const [userData, setUserData] = useState<any>(null);

  const displayName = userData?.full_name || user?.name || user?.username || user?.email || 'Student';
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <Card>
            <div className="flex flex-col items-center p-8">
              ...
              <p className="text-gray-500 capitalize mt-1">{user.role || 'Student'}</p>
            </div>
          </Card>
